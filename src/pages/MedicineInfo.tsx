...
const MedicineInfo = () => {
        ...
        getMedicine(medicine_id)
            .then(data => {
                if (data === null) {
                    ...
                    setName(data.medicine.name ? data.medicine.name : '')
    ...
    const changeCount = (id: string, count: number) => {
        ...
        const accessToken = localStorage.getItem('access_token');
        axiosAPI.put(`/medicines/change_count/${id}`, { count: count }, { headers: { 'Authorization': `Bearer ${accessToken}`, } })
            .then(() => getData())
    }
                ...
                <Navbar>
                    <Nav>
                        ...
                        <Nav.Item className="nav-link p-0 text-dark">
                            {`${medicine.name !== null ? medicine.name : 'Новое лекарство'}`}
                        </Nav.Item>
                    </Nav>
                </Navbar>
                                ...
                                <Form.Group className='mb-1'>
                                    ...
                                    <InputGroup>
                                        ...
                                        {edit && <Button
                                            variant='danger'
                                            onClick={() => {
                                                setName(medicine.name ? medicine.name : '');
                                                setEdit(false)
                                            }}>
                                            Отменить
                                        </Button>}
                                        ...
                                        {medicine.status == 'черновик' &&
                                            <>
                                                ...
                                                <InputGroup className="mt-1">
                                                    <Button variant="outline-danger" onClick={() => changeCount(component.uuid, component.count - 1)}>
                                                        -
                                                    </Button>
                                                    <Form.Control
                                                        ...
                                                        type='number'
                                                        onChange={(e) => changeCount(component.uuid, parseInt(e.target.value))}
                                                    />
                                                    <Button variant="outline-success" onClick={() => changeCount(component.uuid, component.count + 1)}>
                                                        +
                                                    </Button>
                                                </InputGroup>
                                            </>}
